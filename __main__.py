...
def hotword_detection():
    wake_word.wake_word_detection(music_file='features\chime.wav', model=[
                                  'voice_model\jarvis_windows.ppn', 'voice_model\hey-jarvis_windows.ppn'])
...
def background_music(signal):
    if signal == 'start':
        ws.PlaySound('music\IronMan_Theme_Song.wav',
                     ws.SND_ASYNC | ws.SND_ALIAS)
...
def main(count=0):
    while True:
        ...
        count += 1
        text = jarvis_voice_recognise(ptlimit=6)
        ans = rep.reply(text)
        jarvis_speak(ans)

        print('This is trial number "{}" at "{}"'.format(
            count, datetime.datetime.now()))
        save_logs('This is trial number "{}" at "{}"'.format(
            count, datetime.datetime.now()), 'debug')
